...
CLIENT_ID = json.loads(
    open('/var/www/project/project/client_secrets.json', 'r').read())['web']['client_id']
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('/var/www/project/project/client_secrets.json', scope='')
...
def showCategory():
    ...
    if 'username' not in login_session:
        ...
        return render_template('publicCategories.html', categories=
...
def newCategory():
    if 'username' not in login_session:
        flash("You must be logged in for that page")
        return redirect('/categories/')
    if request.method == 'POST':
        newcategory = Category(
            name=request.form['name'], user_id=login_session['user_id'])
        session.add(newcategory)
        flash('Category %s Successfully Created' % newcategory.name)
        session.commit()
        return redirect(url_for('showCategory'))
    else:
...
def showItem(category_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        if 'username' not in login_session:
            ...
            category=category, creator=creator,STATE=state)
        return render_template('publicItem.html', items=items,
        category=category, creator=creator,STATE=None)
    else:
